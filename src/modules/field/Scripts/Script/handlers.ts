...
export const OPCODE_HANDLERS: Record<Opcode, HandlerFuncWithPromise> = {
  ...
  CAL: ({ currentOpcode, STACK }) => {
    ...
    if (currentOpcode.param === 0) {
      STACK.push((value1 + value2) | 0)
    } else if (currentOpcode.param === 1) {
      STACK.push((value1 - value2) | 0)
    } else if (currentOpcode.param === 2) {
      STACK.push(Math.imul(value1, value2))
      ...
      STACK.push(~value2)
    } else if (currentOpcode.param === 16) {
      STACK.push(value1 >> value2)
    } else if (currentOpcode.param === 17) {
      STACK.push(value1 << value2)
    } else {
  ...
  */
  ...
  MOVECANCEL: ({ scene, STACK }) => {
    ...
    const targetMovementController = targetEntity.userData.movementController as
      | ReturnType<typeof createMovementController>
      | undefined
  ...
  POPM_B: ({ currentOpcode, STACK }) => {
    const value = STACK.pop() as number
    console.log('POPM_B', currentOpcode.param, value)
