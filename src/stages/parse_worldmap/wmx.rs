...
impl RgbaImage {
    ...
    fn with_words(mut self, words: &[u16]) -> RgbaImage {
        let palette = palette_rgba_from_words(words);
        for (i, rgba) in palette.iter().enumerate().take((self.width * self.height) as usize) {
...
fn parse_section40(section: &[u8]) -> Vec<PaletteAnimation> {
    ...
    offsets
        ...
        .map(|&record_offset| {
            ...
            let rel_offsets: Vec<usize> =
                (0..frame_count).map(|_| reader.read_u32() as usize).collect();
            ...
            let frames = rel_offsets
                ...
                .map(|&rel| {
                    ...
                    PaletteAnimationFrame {
                        palette_data: section
                            .get(frame_offset..end)
                            .unwrap_or(&[])
                            .to_vec(),
                    }
                })
...
fn parse_section16(section: &[u8]) -> Vec<AnimatedTextureDescriptor> {
    ...
    offsets
        ...
        .map(|&offset| {
            ...
            let frame_offsets = (0..frame_count).map(|_| reader.read_u32() as usize).collect();
...
fn polygon_uv(poly: &WmxPolygon, u: u8, v: u8) -> [f32; 2] {
    if poly.is_water() {
        return [u as f32 / SEA_COMPOSITE_W as f32, v as f32 / SEA_COMPOSITE_H as f32];
    }
    if poly.is_road() {
        return [u as f32 / ROAD_COMPOSITE_W as f32, v as f32 / ROAD_COMPOSITE_H as f32];
    }
...
fn build_segment_primitives(segment: &WmxSegment, seg_idx: usize) -> Vec<Primitive> {
    ...
    for (block_idx, block) in segment.blocks.iter().enumerate() {
        ...
        for poly in &block.polygons {
            ...
            let mut verts: [([f32; 3], [f32; 2], [u8; 4]); 3] =
                [([0.0; 3], [0.0; 2], [0; 4]); 3];
...
fn write_tile_glb(
    ...
    if has[0] {
        let tex = add_external_texture(&mut images, &mut textures, ATLAS_LAND_FILENAME, "atlas_land");
        register(
            &mut materials,
            &mut material_lookup,
            MaterialKind::Land,
    ...
    if has[2] {
        let tex = add_external_texture(&mut images, &mut textures, ATLAS_ROAD_FILENAME, "atlas_road");
        register(
            &mut materials,
            &mut material_lookup,
            MaterialKind::Road,
    ...
    if has[1] {
        if let Some(anim) = build_sea_uv_animation(
            &mut builder,
            &material_lookup,
            sea_frame_count,
            node_idx,
...
fn build_material(name: &str, tex_idx: usize, alpha_mode: &str, uv_scale: Option<f64>) -> JsonValue {
    ...
    pbr.set("baseColorFactor", JsonValue::float_array(&[1.0, 1.0, 1.0, 1.0]));
...
fn primitive_to_gltf(prim: &Primitive, builder: &mut BufferBuilder, material_idx: usize) -> JsonValue {
    let pos_min: Vec<f64> = (0..3)
        .map(|i| prim.positions.iter().map(|p| p[i] as f64).fold(f64::INFINITY, f64::min))
        .collect();
    let pos_max: Vec<f64> = (0..3)
        .map(|i| prim.positions.iter().map(|p| p[i] as f64).fold(f64::NEG_INFINITY, f64::max))
        .collect();
...
fn segment_extras(seg_idx: usize, segment: &WmxSegment, is_variant: bool) -> JsonValue {
    ...
    if !is_variant {
        extras.set("segment_col", JsonValue::int((seg_idx % SEGMENT_GRID_COLS) as i64));
        extras.set("segment_row", JsonValue::int((seg_idx / SEGMENT_GRID_COLS) as i64));
    }
    extras
}
...
fn build_gltf_json(
    ...
    asset.set("generator", JsonValue::string("worldmap_models wmx exporter"));
    asset.set("version", JsonValue::string("2.0"));
    root.set("asset", asset);

    root.set("bufferViews", JsonValue::array(builder.buffer_views.clone()));
...
pub fn export(
    ...
    let world_tims =
        parse_tim_archive(section37, "WorldTex").context("parsing wmset Section37 (world textures)")?;
    let road_tims =
        parse_tim_archive(section38, "RoadTex").context("parsing wmset Section38 (road textures)")?;
    ...
    for (seg_idx, segment) in segments.iter().enumerate() {
        ...
        write_tile_glb(&out_path, seg_idx, segment, &primitives, is_variant, sea_frame_count)?;
    }
